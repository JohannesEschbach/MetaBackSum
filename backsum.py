...
def parse_args():
    ...
    parser.add_argument(
        "--back_model_output_dir", type=str, default=None, help="Where to store the final backward model."
    )
    parser.add_argument(
    ...
    parser.add_argument("--back_model_generate_mode", type=str, default="softmax", choices=["softmax", "autoregressive"])
    parser.add_argument("--train_mode", type=str, choices=["base", "nometa", "metahard", "metadist"])
...
def main():
    ...
    if BACK: mono_dataset = load_dataset("text", data_files=args.mono_dataset_path)
    
    config = AutoConfig.from_pretrained(args.model_name_or_path)
    if BACK: back_config = AutoConfig.from_pretrained(args.back_model_path)
    ...
    train_dataset = processed_datasets["train"].select(
        random.sample(range(len(processed_datasets["train"])), int(args.dataset_perc * len(processed_datasets["train"])))
    )
    
    #Reduce evaluation-set size to speed things up 
    eval_dataset = processed_datasets["validation"].select(
        random.sample(range(len(processed_datasets["validation"])), int(0.25 * len(processed_datasets["validation"])))
    )    
    ...
    if BACK: mono_data_collator = DataCollatorForSeq2Seq(
        tokenizer,
        model=back_model,
        label_pad_token_id=-100,
        pad_to_multiple_of=8 if accelerator.use_fp16 else None,
    ...
    if BACK: mono_dataloader = DataLoader(
    ...
    if BACK: back_optimizer = AdamW(back_model.parameters(), lr=args.learning_rate*0.1) #10 times lower lr 
    ...
    if BACK: mono_dataloader, back_model, back_optimizer = accelerator.prepare(
        mono_dataloader,
        back_model,
        back_optimizer,
    )
    ...
    if BACK: back_lr_scheduler = get_scheduler(
        name=args.lr_scheduler_type,
    ...
    if BACK: mono_iterator = iter(mono_dataloader)
    for epoch in range(args.num_train_epochs):        
        model.train()
        if BACK: back_model.train()
        for batch in train_dataloader:
            ...
            if BACK:
                ...
                softmax_probs = torch.nn.functional.gumbel_softmax(backward_logits, hard=True, dim=-1)
            ...
            if BACK and args.train_mode != "nometa":
                ...
                reward = torch.nan_to_num(reward, nan=0.0)  #Happens for some reason in first iteration?
                advantage = reward-reward_mv_avg
                reward_mv_avg += advantage/(completed_steps + 1) #Does that work with Accelerate just like that? No race conditions?
                if args.train_mode == "metahard":
                    softmax_probs=torch.clamp(softmax_probs, min=1e-9, max=1 - 1e-9) #Max was unnecessary. Min is important due to 0*log(0) of entropy calculation
                ...
                if completed_steps % 500 == 0 or (completed_steps % 40 == 0 and completed_steps < 200):
                    back_model.eval()
                    with torch.no_grad():
                        generated_tokens = accelerator.unwrap_model(back_model).generate(
    ...
    if args.back_model_output_dir is not None:
        ...
        unwrapped_model.save_pretrained(args.back_model_output_dir, save_function=accelerator.save)
        if accelerator.is_main_process:
